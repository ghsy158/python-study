...
from tutorials.items import HouseItem, CjHouseItem
...
class BjljiaSpider(CrawlSpider):
    ...
    def parse_one_house_info(self, response):
        ...
        item['community_name'] = content.css("div[class='aroundInfo']").css("div[class='communityName']").css(
        ...
        item['house_begin_sell'] = intro_content.css("div[class='transaction']").css("div[class='content'] li::text")[
            0].extract()
        item['house_transacton'] = intro_content.css("div[class='transaction']").css("div[class='content'] li::text")[
            1].extract()
        item['house_purpose'] = intro_content.css("div[class='transaction']").css("div[class='content'] li::text")[
            3].extract()
        item['house_full_five'] = intro_content.css("div[class='transaction']").css("div[class='content'] li::text")[
            4].extract()
        item['house_unique'] = intro_content.css("div[class='transaction']").css("div[class='content'] li::text")[
            6].extract()
        item['morgage'] = intro_content.css("div[class='transaction']").css("div[class='content'] li::text")[
            8].extract()
        item['sell_flag'] = 0
    ...
    def parse_url(self, response):
        links = response.css('a::attr(href)').extract()
        for link in [link for link in links if link.startswith('http://bj.lianjia.com/ershoufang/') or link.startswith("http://bj.lianjia.com/chengjiao/")]:
            yield scrapy.Request(url=link, callback=self.parse_url)
